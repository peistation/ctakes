...
+
- *analysis_engine/ClearTrainerAE.xml* and *ClearTrainer\*Aggregate.xml*
+
...
This analyzer is an alternative to using LVG output from cTAKES.
...
in part because it is small enough to with {osp-short}.
...
you want to rely on other components.  Here are two methods of training.
...
- To create a model using {osp-short} POS tags and lemmas with Eclipse:
	. Create a +<your-data>.min+ file from +<your-data>.dep+ 
