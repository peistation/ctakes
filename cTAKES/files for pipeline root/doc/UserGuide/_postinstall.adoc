...
(CPE) to process multiple documents in one batch. To run the CPE:
...
TIP: This command includes all possible classes {osp-short} may make
use of, thus you can use them to run any CPE shipped in the
package. You can also take the classpath and run CVD, etc. We will
refer to this classpath as +{osp-cp}+.

TIP: Eclipse users may use
``UIMA_CPE_GUI\--clinical_documents_pipeline'' launch. Users who
compiled from source can use the ant target ``testcpe''.
--
+
