...
public class OverlapAnnotator extends JTextAnnotator_ImplBase
{
			...
			AnnotatorConfigurationException
	{
		...
		{
			String classname;
			classname = (String) annotCtx.getConfigParameterValue("A_ObjectClass");
			...
			throws AnnotatorProcessException
	{
		...
		List aAnnotList = storeAnnotationsToList(indexes, iv_aAnnotType);
		List bAnnotList = storeAnnotationsToList(indexes, iv_bAnnotType);

		Iterator aAnnotItr = aAnnotList.iterator();
		while (aAnnotItr.hasNext())
		{
			Annotation aAnnot = (Annotation) aAnnotItr.next();

			// for each A, iterate over B annotations
			Iterator bAnnotItr = bAnnotList.iterator();
			while (bAnnotItr.hasNext())
			{
				Annotation bAnnot = (Annotation) bAnnotItr.next();

				// get type of overlap between A and B
				BitSet jcasOverlapType = getOverlapType(aAnnot, bAnnot);

				// determine if overlap is one of the type(s) requested - perform AND operation between bitsets
	 ...
	 * @return
	 */
	private List storeAnnotationsToList(JFSIndexRepository indexes,
			int annotType)
	{
		List l = new ArrayList();
		Iterator annotItr = indexes.getAnnotationIndex(annotType).iterator();
	 ...
	 * @param a
	 * @param b
	 * @return BitSet with the bit corresponding to the overlap type set to true.
	 */
