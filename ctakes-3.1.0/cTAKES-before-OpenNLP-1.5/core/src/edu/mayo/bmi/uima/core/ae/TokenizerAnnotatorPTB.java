...
public class TokenizerAnnotatorPTB extends JCasAnnotator_ImplBase
{
    ...
    private static boolean MEMORY_INTENSIVE = true;
        ...
        while (segmentItr.hasNext()) {
            ...
            if (!skipSegmentsSet.contains(segmentID)) { 
                try {
                    ...
                    if (MEMORY_INTENSIVE) System.gc();
                } catch (java.lang.OutOfMemoryError e) {
                    logger.info("Caught exception at " + new Date());
                    throw e;
    ...
    protected void annotateRange(JCas jcas, int beginPos, int endPos) throws AnnotatorProcessException {
        ...
        for (int i = beginPos; i<endPos; i++) {
        	...
        	NewlineToken nta;
        	if (i+1<endPos && docText.charAt(i+1)==LF) {
        ...
        while (baseTokenIter.hasNext()) {
            ...
            if (bta.getBegin()>=beginPos && bta.getBegin()<endPos) {
