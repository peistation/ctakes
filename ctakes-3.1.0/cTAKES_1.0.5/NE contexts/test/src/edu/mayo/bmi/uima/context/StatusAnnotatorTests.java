...
public class StatusAnnotatorTests {
	...
	public void testNegationExamples() throws ResourceInitializationException, AnalysisEngineProcessException {
		...
		TestUtil.testConfigParam(uimaContext, descriptor, ContextAnnotator.CONTEXT_ANALYZER_CLASS_PARAM, "edu.mayo.bmi.uima.context.status.StatusContextAnalyzer");
		TestUtil.testConfigParam(uimaContext, descriptor, ContextAnnotator.CONTEXT_HIT_CONSUMER_CLASS_PARAM, "edu.mayo.bmi.uima.context.status.StatusContextHitConsumer");
		...
		NamedEntity ne1 = addNE(jCas, 104, 121, "sinus bradycardia");
		NamedEntity ne2 = addNE(jCas, 104, 109, "sinus");
		NamedEntity ne3 = addNE(jCas, 110, 121, "bradycardia");
		NamedEntity ne4 = addNE(jCas, 123, 146, "left atrial enlargement");
		NamedEntity ne5 = addNE(jCas, 135, 146, "enlargement");
		NamedEntity ne6 = addNE(jCas, 166, 173, "infarct");
		NamedEntity ne7 = addNE(jCas, 175, 176, "T");
		NamedEntity ne8 = addNE(jCas, 182, 193, "abnormality");
		NamedEntity ne9 = addNE(jCas, 212, 220, "ischemia");
		statusAnnotator.process(jCas);
		assertEquals(3, ne1.getStatus()); // 3 means probable, possible, etc
		assertEquals(3, ne2.getStatus());
		assertEquals(3, ne3.getStatus());
		assertEquals(3, ne4.getStatus());
		assertEquals(3, ne5.getStatus());
		assertEquals(3, ne6.getStatus());
		assertEquals(3, ne7.getStatus());
		assertEquals(3, ne8.getStatus());
		assertEquals(0, ne9.getStatus()); // 0 since window size prevents "possible" from being applied to "ischemia"
		...
		statusAnnotator.process(jCas);
		assertEquals(0, ne1.getStatus());
		assertEquals(0, ne2.getStatus());
		assertEquals(3, ne3.getStatus());
		assertEquals(3, ne4.getStatus());
		...
		statusAnnotator.process(jCas);
		assertEquals(1, ne1.getStatus()); // 1 means History of
		...
		statusAnnotator.process(jCas);
		assertEquals(1, ne1.getStatus());
		assertEquals(1, ne2.getStatus());
		...
		statusAnnotator.process(jCas);
		assertEquals(2, ne1.getStatus()); // 2 means Family history of
		...
		statusAnnotator.process(jCas);
		assertEquals(2, ne1.getStatus());
		assertEquals(2, ne2.getStatus());
		assertEquals(2, ne3.getStatus());
